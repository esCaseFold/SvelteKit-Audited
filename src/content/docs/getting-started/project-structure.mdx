...
- `params` contains any [param matchers](/SvelteKit-Audited/advanced/advanced-routing#matching) your app needs
- `routes` contains the [routes](/SvelteKit-Audited/core-concepts/routing/) of your application. You can also colocate other components that are only used within a single route here
- `app.html` is your page template — an HTML document containing the following placeholders:
  ...
  - `%sveltekit.env.[NAME]%` - this will be replaced at render time with the `[NAME]` environment variable, which must begin with the [`publicPrefix`](https://svelte.dev/docs/kit/configuration#env) (usually `PUBLIC_`), or be defined as a public variable in `src/env` if using [`experimental.explicitEnvironmentVariables`](https://svelte.dev/docs/kit/environment-variables). It will fallback to `''` if not matched.
...
- `hooks.client.js` contains your client [hooks](/SvelteKit-Audited/advanced/hooks)
- `hooks.server.js` contains your server [hooks](/SvelteKit-Audited/advanced/hooks)
- `service-worker.js` contains your [service worker](/SvelteKit-Audited/advanced/service-workers)
