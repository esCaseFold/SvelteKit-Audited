---
title: Project Types
description: How to configure rendering, how to build and deploy your project.
sidebar:
    order: 3
---
...
If you are using an external backend, you can simply skip over and ignore the parts of the docs talking about `server` files. You may also want to reference [the FAQ about how to make calls to a separate backend](/appendix/faq#how-do-i-use-a-different-backend-api-server).

## Serverless app

SvelteKit apps are simple to run on serverless platforms. [The default zero config adapter](/build-and-deploy/adapter-auto) will automatically run your app on a number of supported platforms or you can use [`adapter-vercel`](https://svelte.dev/docs/kit/adapter-vercel), [`adapter-netlify`](https://svelte.dev/docs/kit/adapter-netlify), or [`adapter-cloudflare`](https://svelte.dev/docs/kit/adapter-cloudflare) to provide platform-specific configuration. And [community adapters](/https://svelte.dev/packages#sveltekit-adapters) allow you to deploy your application to almost any serverless environment. Some of these adapters such as [`adapter-vercel`](https://svelte.dev/docs/kit/adapter-vercel) and [`adapter-netlify`](https://svelte.dev/docs/kit/adapter-netlify) offer an `edge` option, to support [edge rendering](https://svelte.dev/docs/kit/glossary#Edge) for improved latency.
...
SvelteKit has full support for [service workers](/advanced/service-workers) allowing you to build many types of applications such as offline apps and [progressive web apps](https://svelte.dev/docs/kit/glossary#PWA).
