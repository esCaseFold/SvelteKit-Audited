...
These APIs are available in all modern browsers and in many non-browser environments like Cloudflare Workers, Deno, and Vercel Functions. During development, and in [adapters](/SvelteKit-Audited/build-and-deploy/adapters) for Node-based environments (including AWS Lambda), they’re made available via polyfills where necessary (for now, that is — Node is rapidly adding support for more web standards).
...
:::note
A special version of `fetch` is available in [`load`](/SvelteKit-Audited/core-concepts/loading-data) functions, [server hooks](/SvelteKit-Audited/advanced/hooks), [API routes](/SvelteKit-Audited/core-concepts/routing#server) and [remote functions](https://svelte.dev/docs/kit/remote-functions) for invoking endpoints directly during server-side rendering, without making an HTTP call, while preserving credentials. (To make credentialled fetches in server-side code outside `load`, you must explicitly pass `cookie` and/or `authorization` headers.) It also allows you to make relative requests, whereas server-side `fetch` normally requires a fully qualified URL.
:::
...
### Request
An instance of [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request) is accessible in [`hooks`](/SvelteKit-Audited/advanced/hooks/) and [`server routes`](/SvelteKit-Audited/core-concepts/routing#server) as `event.request`. It contains useful methods like `request.json()` and `request.formData()` for getting data that was posted to an endpoint.
### Responce
...
### Headers
The [`Headers`](https://developer.mozilla.org/en-US/docs/Web/API/Headers) interface allows you to read incoming `request.headers` and set outgoing `response.headers`. For example, you can get the request.headers as shown below, and use the [`json` convenience function](/SvelteKit-Audited/reference/@sveltejs-kit/#json) to send modified `response.headers`:
...
## URL APIs
URLs are represented by the [`URL`](https://developer.mozilla.org/en-US/docs/Web/API/URL) interface, which includes useful properties like `origin` and `pathname` (and, in the browser, `hash`). This interface shows up in various places — `event.url` in [hooks](/SvelteKit-Audited/advanced/hooks) and [server routes](/SvelteKit-Audited/core-concepts/routing/#server), [`page.url`](https://svelte.dev/docs/kit/$app-state) in [pages](/SvelteKit-Audited/core-concepts/routing/#page), `from` and `to` in [`beforeNavigate`](https://svelte.dev/docs/kit/$app-navigation#beforeNavigate) and [`afterNavigate`](https://svelte.dev/docs/kit/$app-navigation#afterNavigate) and so on.
### URLSearchParams
