...
You can [disable automatic registration](https://svelte.dev/docs/kit/configuration#serviceWorker) if you need to register the service worker with your own logic. The default registration looks something like this:
...
SvelteKit calls [`registration.update()`](https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorkerRegistration/update) only as part of error recovery — if a route module fails to load or a navigation results in an error status, and [version polling](https://svelte.dev/docs/kit/configuration#version) detects that the app has been redeployed, the service worker is updated before SvelteKit falls back to a full-page navigation.
