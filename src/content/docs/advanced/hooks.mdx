...
This function runs every time the SvelteKit server receives a [request](/getting-started/web-standards/#request) — whether that happens while the app is running, or during [prerendering](/core-concepts/page-options/#prerender) — and determines the [response](/getting-started/web-standards/#responce). It receives an `event` object representing the request and a function called `resolve`, which renders the route and generates a `Response`. This allows you to modify response headers or bodies, or bypass SvelteKit entirely (for implementing routes programmatically, for example).
...
You can define multiple `handle` functions and execute them with the [`sequence`](/reference/@sveltejs-kit-hooks#sequence) helper function.
...
This function allows you to modify (or replace) the result of an [`event.fetch`](/core-concepts/loading-data#making-fetch-requests) call that runs on the server (or during prerendering) inside an endpoint, `load`, `action`, `handle`, `handleError` or `reroute`.
...
If an [unexpected error](/advanced/errors#unexpected-errors) is thrown during loading, rendering, or from an endpoint, this function will be called with the `error`, `event`, `status` code and `message`. This allows for two things:
...
This function is not called for _expected_ errors (those thrown with the [`error`](/reference/@sveltejs-kit#error) function imported from `@sveltejs/kit`).
