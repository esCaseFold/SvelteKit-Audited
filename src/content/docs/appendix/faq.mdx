...
See [the documentation regarding project types](/SvelteKit-Audited/getting-started/project-types) for more details.
...
You can use [`event.fetch`](/SvelteKit-Audited/core-concepts/loading-data#making-fetch-requests) to request data from an external API server, but be aware that you would need to deal with [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS), which will result in complications such as generally requiring requests to be preflighted resulting in higher latency. Requests to a separate subdomain may also increase latency due to an additional DNS lookup, TLS setup, etc. If you wish to use this method, you may find [`handleFetch`](/SvelteKit-Audited/advanced/hooks#handlefetch) helpful.
...
How to set up rewrites in production will depend on your deployment platform. If rewrites aren't an option, you could alternatively add an [API route](/SvelteKit-Audited/core-concepts/routing#server):
