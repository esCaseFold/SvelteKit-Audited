...
Before a [`+page.svelte`](/SvelteKit-Audited/core-concepts/routing/#pagesvelte) component (and its containing [`+layout.svelte`](/SvelteKit-Audited/core-concepts/routing/#layoutsvelte) components) can be rendered, we often need to get some data. This is done by defining `load` functions.
...
A load function in a `+page.js` file runs both on the server and in the browser (unless combined with export const ssr = false, in which case it will [only run in the browser](/SvelteKit-Audited/core-concepts/page-options#ssr)). If your load function should always run on the server (because it uses private environment variables, for example, or accesses a database) then it would go in a +page.server.js instead.
...
By default, universal `load` functions run on the server during SSR when the user first visits your page. They will then run again during hydration, reusing any responses from [fetch requests](#making-fetch-requests). All subsequent invocations of universal `load` functions happen in the browser. You can customize the behavior through [page options](/SvelteKit-Audited/core-concepts/page-options). If you disable [server-side rendering](/SvelteKit-Audited/core-concepts/page-options#ssr), you'll get an SPA and universal `load` functions _always_ run on the client.

If a route contains both universal and server `load` functions, the server `load` runs first.

A `load` function is invoked at runtime, unless you [prerender](/SvelteKit-Audited/core-concepts/page-options#prerender) the page — in that case, it's invoked at build time.
...
A server `load` function must return data that can be serialized with [devalue](https://github.com/rich-harris/devalue) — anything that can be represented as JSON plus things like `BigInt`, `Date`, `Map`, `Set` and `RegExp`, or repeated/cyclical references — so that it can be transported over the network. Your data can include [promises](#streaming-with-promises), in which case it will be streamed to browsers. If you need to serialize/deserialize custom types, use [transport hooks](/SvelteKit-Audited/advanced/hooks#transport).
...
A server `load` function can get and set [`cookies`](/SvelteKit-Audited/reference/@sveltejs-kit#cookies).
...
If an error is thrown during `load`, the nearest [`+error.svelte`](/SvelteKit-Audited/core-concepts/routing#error) will be rendered. For [_expected_](/SvelteKit-Audited/advanced/errors#expected-errors) errors, use the `error` helper from `@sveltejs/kit` to specify the HTTP status code and an optional message:
...
If an [_unexpected_](/SvelteKit-Audited/advanced/errors#unexpected-errors) error is thrown, SvelteKit will invoke [`handleError`](/SvelteKit-Audited/advanced/hooks#handleerror) and treat it as a 500 Internal Error.
...
<Aside type="note" title="Previous Version Note">
[In SvelteKit 1.x](/SvelteKit-Audited/appendix/migrating-to-sveltekit-v2/#redirect-and-error-are-no-longer-thrown-by-you) you had to `throw` the `redirect` yourself
</Aside>
...
<Aside type="note" title="Previous Version Note">
[In SvelteKit 1.x](/SvelteKit-Audited/appendix/migrating-to-sveltekit-v2/#top-level-promises-are-no-longer-awaited) top-level promises were automatically awaited, only nested promises were streamed.
</Aside>
...
- It declared a dependency on a specific URL via [`fetch`](#making-fetch-requests) (universal load only) or [`depends`](/SvelteKit-Audited/reference/@sveltejs-kit/#loadevent), and that URL was marked invalid with [`invalidate(url)`](https://svelte.dev/docs/kit/$app-navigation#invalidate)
- All active `load` functions were forcibly rerun with [`invalidateAll()`](https://svelte.dev/docs/kit/$app-navigation#invalidateAll)

`params` and `url` can change in response to a `<a href="..">` link click, a [`<form>` interaction](/SvelteKit-Audited/core-concepts/form-actions/#get-vs-post), a [`goto`](https://svelte.dev/docs/kit/$app-navigation#goto) invocation, or a [`redirect`](/SvelteKit-Audited/reference/@sveltejs-kit/#redirect-1).
...
- Use [hooks](/SvelteKit-Audited/advanced/hooks) to protect multiple routes before any `load` functions run
...
```
Now, you can call `requireLogin` in any `load` function (or [form action](/SvelteKit-Audited/core-concepts/form-actions), for example) to guarantee that the user is logged in:
