...
```svelte ins="action="?/login"" ins={12}
...
:::
```svelte ins={3-4} ins="value={form?.email ?? ''}"
// src/routes/login/+page.svelte
<form method="POST" action="?/login">
...
Redirects (and errors) work exactly the same as in [`load`](/SvelteKit-Audited/core-concepts/loading-data/#redirects):

<Tabs syncKey="languages">
  <TabItem label="JavaScript" icon="seti:javascript">
	```js ins="redirect" ins="url" ins={23-25}
	// src/routes/login/+page.server.js
...
<Tabs syncKey="languages">
  ...
  <TabItem label="TypeScript" icon="seti:tsconfig">
	```svelte ins={3-4} ins="use:enhance"
	// src/routes/login/+page.svelte
	<script lang="ts">
		import { enhance } from '$app/forms';
		import type { PageProps } from './$types';
...
<Tabs syncKey="languages">
  <TabItem label="JavaScript" icon="seti:javascript">
	```svelte ins="applyAction" ins={13-17}
	// src/routes/login/+page.svelte
	<script lang="ts">
...
Form actions are the preferred way to send data to the server, since they can be progressively enhanced, but you can also use [`+server.js`](/SvelteKit-Audited/core-concepts/routing#server) files to expose (for example) a JSON API. Here's how such an interaction could look like:
...
Submitting this form will navigate to `/search?q=...` and invoke your load function but will not invoke an action. As with `<a>` elements, you can set the [`data-sveltekit-reload`](/SvelteKit-Audited/advanced/link-options/#data-sveltekit-reload), [`data-sveltekit-replacestate`](/SvelteKit-Audited/advanced/link-options/#data-sveltekit-replacestate), [`data-sveltekit-keepfocus`](/SvelteKit-Audited/advanced/link-options/#data-sveltekit-keepfocus) and [`data-sveltekit-noscroll`](/SvelteKit-Audited/advanced/link-options/#data-sveltekit-noscroll) attributes on the `<form>` to control the router's behaviour.
