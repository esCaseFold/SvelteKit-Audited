...
Redirects (and errors) work exactly the same as in [`load`](/core-concepts/loading-data/#redirects):
...
Submitting this form will navigate to `/search?q=...` and invoke your load function but will not invoke an action. As with `<a>` elements, you can set the [`data-sveltekit-reload`](/advanced/link-options/#data-sveltekit-reload), [`data-sveltekit-replacestate`](/advanced/link-options/#data-sveltekit-replacestate), [`data-sveltekit-keepfocus`](/advanced/link-options/#data-sveltekit-keepfocus) and [`data-sveltekit-noscroll`](/advanced/link-options/#data-sveltekit-noscroll) attributes on the `<form>` to control the router's behaviour.
